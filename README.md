...
This is what the original code looked like which generated a single button that, upon clicking, would return a randomly sampled narrative relating to the selection the user has made in terms of product involved in the injury. The variable labelled `selected` (line 10) contains a Shiny reactive expression which subsets the dataframe containing *all* injuries to only those selected by the user. The selection is made when the user clicks on a product in the drop-down list.   
